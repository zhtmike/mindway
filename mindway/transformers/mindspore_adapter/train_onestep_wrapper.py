...
class TrainOneStepWrapper(nn.Cell):
    ...
    def __init__(
        ...
        if clip_grad.lower() in ("norm", "l2norm", "l2_norm", "global", "global_norm", "total", "total_norm"):
            ...
            if self.is_zero:
                from mindway.transformers.mindspore_adapter.clip_grad import clip_grad_norm_for_zero

                clip_grad_fn = clip_grad_norm_for_zero
            else:
                from mindway.transformers.mindspore_adapter.clip_grad import clip_grad_norm

                clip_grad_fn = clip_grad_norm
        elif clip_grad.lower() in ("local", "value"):
            from mindway.transformers.mindspore_adapter.clip_grad import clip_grad_value
